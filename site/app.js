...
document.addEventListener("input",e=>{const id=e.target.dataset.value,lim=e.target.dataset.limit;if(id){values[id]=num(e.target.value);refresh()}else if(lim){const[mid,key]=lim.split(":");overrides[mid]={...(overrides[mid]||{}),[key]:num(e.target.value)};refresh()}else if(e.target.matches("#asset-name,#period"))save()});
document.addEventListener("change",e=>{if(e.target.matches("#search,#category,#issues-only,#show-limits"))renderTable();if(e.target.id==="file-input"&&e.target.files[0]){importFile(e.target.files[0]);e.target.value=""}});
